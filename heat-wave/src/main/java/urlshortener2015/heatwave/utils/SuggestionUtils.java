...
public class SuggestionUtils {
	...
	public static ArrayList<Suggestion> sugerenciasFromAPIs(ArrayList<Suggestion> lista, String customTag, ShortURLRepository shortURLRepository){
		...
		try {
			...
			while(sugerenciasIngles<3 && sugerencias.length>(i+2)){
				if(shortURLRepository.findByHash(sugerencias[i])==null){
					lista.add(new Suggestion(sugerencias[i]));
					sugerenciasIngles++;
				}
				// son en posiciones impares (en las pares hay comas)
				i+=2;
			}
			response = restTemplate.getForEntity("http://thesaurus.altervista.org/thesaurus/v1?word="+customTag+"&language=en_US&key=n2HXWUfppJ1S60lje6Km", String.class);
			...
			response = restTemplate.getForEntity("http://watson.kmi.open.ac.uk/API/term/synonyms?term="+customTag, String.class);
			body = response.getBody();
			sugerencias =body.split("\",\"");
			i=1;
			...
			response = restTemplate.getForEntity("http://dictionaryapi.net/api/definition/"+customTag, String.class);
	...
	public static boolean esta(ArrayList<Suggestion> lista, String recomendacion){
		boolean esta=false;
